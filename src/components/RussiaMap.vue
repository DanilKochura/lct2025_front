...
<!--      ></l-geo-json>-->
      ...
      <template v-if="showPoints && selectedRegion">
        <l-geo-json
            v-for="data in filteredPoints"
            :key="`point-${data.properties.flightId}`"
            :geojson="data"
...
// Загрузка GeoJSON с GitHub
// const loadRealGeoJson = async () => {
//   try {
//     const response = await fetch('https://birthday.imdibil.ru/russiaRegions.geojson')
//     const data = await response.json()
//     regions.value = data.features
//     console.log('Загружено регионов:', regions.value.length)
//   } catch (error) {
//     console.error('Ошибка загрузки GeoJSON:', error)
//     // Fallback - создаем простые регионы
...
const getStyle = (region) => {
  ...
  if (flightCount > 0) {
    if (flightCount < 300) fillColor = '#bbf7d0'
    else if (flightCount < 700) fillColor = '#4ade80'
...
const processedFlights = computed(() =>
    processFlightData(props.flightsData.flights || [])
)

const pointsData = computed(() =>
    createPointsGeoJSON(processedFlights.value)
)

const routesData = computed(() =>
    createRoutesGeoJSON(processedFlights.value)
)

const zonesData = computed(() =>
    createZonesGeoJSON(processedFlights.value)
)

// Функция для получения полета по фиче
const getFlightByFeature = (feature) => {
  return processedFlights.value.find(f => f.sid === feature.properties.flightId)
}
...
const filteredPoints = computed(() => {
  console.log('here')
  if (!selectedRegion.value || !pointsData.value.features.length) {
    console.log("Error!!!!")
    return [];
  }
  ...
  console.log(pointsData.value.features)
  return pointsData.value.features.filter(feature => {
    // Проверяем разные возможные варианты хранения regionId в свойствах
    return feature.properties.region === regionId;
        // feature.properties.regionId === regionId ||
...
onMounted(() => {
  setTimeout(() => {
    console.log(props.flightsData.flights)
